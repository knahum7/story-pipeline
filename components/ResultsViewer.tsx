  ...
  return (
    <div className="bg-ink-soft border border-ink-muted rounded-xl p-4 space-y-3 card-hover">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div>
          <span className="text-parchment/30 block mb-1">Hair</span>
          <span className="text-parchment/70">{char.physical_description.hair}</span>
        </div>
        <div>
          <span className="text-parchment/30 block mb-1">Build</span>
          <span className="text-parchment/70">{char.physical_description.build}</span>
        </div>
        <div className="col-span-2">
          <span className="text-parchment/30 block mb-1">Style</span>
      ...
      <div className="bg-ink border border-ink-muted rounded-lg p-3">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <span className="text-xs text-amber-film font-mono">Voice</span>
        </div>
      ...
      <PromptBox label="Portrait Prompt" prompt={char.image_generation_prompt} />
      {char.character_reference_sheet_prompt && (
        <PromptBox label="Reference Sheet Prompt" prompt={char.character_reference_sheet_prompt} />
      )}
    </div>
  );
}
    ...
    <div className="bg-ink-soft border border-ink-muted rounded-xl overflow-hidden card-hover">
      <div className="px-4 py-3 border-b border-ink-muted flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          />
          {scene.turning_point && <Pill text="turning point" color="amber" />}
        </div>
      </div>
      ...
      <div className="p-4 space-y-3">
        ...
        {scene.subtext && (
          <div className="bg-scene-blue/20 border border-scene-blue/30 rounded-lg px-3 py-2">
            <span className="text-xs text-blue-400 font-mono block mb-1">SUBTEXT</span>
        ...
        {/* Key visual */}
        <div className="bg-ink border border-ink-muted rounded-lg p-3">
          <span className="text-xs text-parchment/30 font-mono block mb-1">KEY VISUAL</span>
        ...
        {scene.dialogue && scene.dialogue.length > 0 && (
          <div className="space-y-2">
            <span className="text-xs text-parchment/30 font-mono">DIALOGUE</span>
        ...
        {/* Camera */}
        <div>
          <span className="text-xs text-parchment/30 font-mono block mb-1">CAMERA</span>
          <p className="text-xs text-parchment/60">{scene.camera_direction}</p>
        </div>

        <PromptBox label="Scene Image Prompt" prompt={scene.image_generation_prompt} />

        <div>
          <span className="text-xs text-parchment/30 font-mono block mb-1">ANIMATION NOTES</span>
...
export default function ResultsViewer({ data }: ResultsViewerProps) {
  ...
  const tabs = [
    ...
    { id: "characters", label: `Characters (${data.characters?.length || 0})`, icon: Users },
    { id: "scenes", label: `Scenes (${data.scenes?.length || 0})`, icon: Clapperboard },
    { id: "settings", label: `Settings (${data.settings?.length || 0})`, icon: MapPin },
    { id: "production", label: "Production", icon: Music },
  ] as const;
    ...
    <div className="space-y-6 animate-fade-up">
      ...
      <div className="bg-gradient-to-r from-amber-film/10 to-scene-blue/10 border border-amber-film/20 rounded-2xl p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs text-amber-film font-mono mb-1 uppercase tracking-widest">Pipeline Generated</p>
            <h2 className="font-display text-3xl font-bold text-parchment italic">{data.story?.title}</h2>
            {data.story?.author && (
              <p className="text-parchment/60 mt-1">by {data.story.author}
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {data.story?.tone?.split(",").map((t: string, i: number) => (
            <Pill key={i} text={t.trim()} />
          ))}
        </div>
      </div>
          ...
          <div className="space-y-4 animate-fade-up">
                ...
                {(["act_1", "act_2", "act_3"] as const).map((act, i) => (
                  <div key={act} className="bg-ink-soft border border-ink-muted rounded-xl p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-mono text-parchment/40 uppercase">Act {i + 1}</span>
                    </div>
                    <p className="text-xs text-parchment/70 leading-relaxed">{data.narrative_arc[act]?.description}</p>
                    <p className="text-xs text-parchment/30 mt-2 font-mono">{data.narrative_arc[act]?.scenes?.length} scenes</p>
                  </div>
                ))}
              </div>
            )}
            ...
            {data.narrative_arc?.central_question && (
              <div className="bg-scene-teal/10 border border-scene-teal/20 rounded-xl p-4">
                <p className="text-xs text-green-400/60 font-mono mb-1 uppercase tracking-wider">Central Question</p>
                <p className="text-sm text-parchment/80 italic">{data.narrative_arc.central_question}</p>
                {data.narrative_arc.answer && (
                  <>
                    <p className="text-xs text-green-400/60 font-mono mt-3 mb-1 uppercase tracking-wider">Answer</p>
            ...
            {data.story?.art_style_direction && (
              <Accordion title="Art Style Direction" defaultOpen>
            ...
            {data.voice_casting_summary && (
              <Accordion title="Voice Casting Summary">
        ...
        {activeTab === "scenes" && (
          <div className="space-y-4 animate-fade-up">
            ...
            {data.flashback_sequences && data.flashback_sequences.length > 0 && (
              <div className="mt-6">
                <h3 className="text-sm font-mono text-blue-400 mb-3 uppercase tracking-wider">Flashback Sequences</h3>
                {data.flashback_sequences.map((fb) => (
                  <div key={fb.id} className="bg-scene-blue/10 border border-scene-blue/20 rounded-xl p-4 mb-3">
                    ...
                    <PromptBox label="Flashback Image Prompt" prompt={fb.image_generation_prompt} />
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="grid gap-4 animate-fade-up">
            {data.settings?.map((setting) => (
              <div key={setting.id} className="bg-ink-soft border border-ink-muted rounded-xl p-4 space-y-3 card-hover">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <span className="text-parchment/30 block mb-1">Mood</span>
                    <span className="text-parchment/70">{setting.mood}</span>
                  </div>
                  <div>
                    <span className="text-parchment/30 block mb-1">Color Palette</span>
                    <span className="text-parchment/70">{setting.color_palette}</span>
                  </div>
                  {setting.sound_environment && (
                    <div className="col-span-2">
                      <span className="text-parchment/30 block mb-1">Sound</span>
                      <span className="text-parchment/70">{setting.sound_environment}</span>
                    </div>
                  )}
                </div>
                <PromptBox label="Setting Image Prompt" prompt={setting.image_generation_prompt} />
              </div>
            ))}
          </div>
        )}
          ...
          <div className="space-y-4 animate-fade-up">
            ...
            {data.music_direction && (
              <div className="bg-ink-soft border border-ink-muted rounded-xl p-4 space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-semibold text-parchment">Music Direction</h3>
                </div>
                <div className="grid grid-cols-3 gap-3 text-xs">
                  <div>
                    <span className="text-parchment/30 block mb-1">Genre</span>
                    <span className="text-parchment/70">{data.music_direction.genre}</span>
                  </div>
                  <div>
                    <span className="text-parchment/30 block mb-1">Tempo</span>
                    <span className="text-parchment/70">{data.music_direction.tempo}</span>
                  </div>
                  <div>
                    <span className="text-parchment/30 block mb-1">Tone</span>
                    <span className="text-parchment/70">{data.music_direction.overall_tone}</span>
                  </div>
                </div>
                <div className="text-xs">
                  <span className="text-parchment/30 block mb-1">Instruments</span>
                  <span className="text-parchment/70">{data.music_direction.suggested_instruments}</span>
                </div>
                {data.music_direction.reference_tracks && (
                  <div className="text-xs">
                    <span className="text-parchment/30 block mb-1">Reference Tracks</span>
                    <span className="text-parchment/70">{data.music_direction.reference_tracks}</span>
                  </div>
                )}
                <PromptBox label="Suno.ai Prompt" prompt={data.music_direction.suno_prompt} />
              </div>
            )}
            ...
            {data.production_notes?.production_order && (
              <div className="bg-ink-soft border border-ink-muted rounded-xl p-4">
                <h3 className="text-sm font-semibold text-parchment mb-3">Production Order</h3>
            ...
            {data.production_notes?.critical_warnings && data.production_notes.critical_warnings.length > 0 && (
              <div className="bg-amber-film/5 border border-amber-film/20 rounded-xl p-4">
                <h3 className="text-xs font-mono text-amber-film uppercase tracking-wider mb-3">Critical Warnings</h3>
